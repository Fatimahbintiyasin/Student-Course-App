...
exports.list = function (req, res) {
    Course.find({ students: req.id }).sort('-created').populate('students', 'firstName lastName fullName').exec((err, courses) => {
if (err) {
        return res.status(400).send({
            message: getErrorMessage(err)
        });
...
exports.courseByID = function (req, res, next, id) {
    Course.findById(id).populate('students', 'firstName lastName fullName').exec((err, course) => {if (err) return next(err);
...
exports.hasAuthorization = function (req, res, next) {
    ...
    if (req.course.students && req.course.students.id !== req.id) {
